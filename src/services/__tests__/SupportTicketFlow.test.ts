...
describe('Integration: Support Ticket Flow', () => {
  ...
  it('should create a ticket and handle message exchange', async () => {
    // 1. Client creates a ticket
    const ticket = await supportService.createTicket('client-123', 'Roberto Oliveira', {
  ...
  it('should filter tickets by client', async () => {
    await supportService.createTicket('c1', 'Client 1', { subject: 'T1', message: 'M1' });
    await supportService.createTicket('c2', 'Client 2', { subject: 'T2', message: 'M2' });
